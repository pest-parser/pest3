...
macro_rules! choice_type {
    ( $name:ident, $( ( $variant:ident, $type:ident, $getter:ident ) ),* $(,)? ) => {
        ...
        impl<'i, $($type, )*> $name<$($type, )*> {
            $(
                /// Try to get variant.
                pub fn $getter(&self) -> $crate::std::Option<&$type> {
                    match self {
                        Self::$variant(val) => $crate::std::Some(val),
                        _ => $crate::std::None,
                    }
                }
            )*
